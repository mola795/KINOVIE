class List < ApplicationRecord
  ...
  def activity_string
    ...
    if time_minutes <= 60
      "#{user.first_name} created \"#{name}\" — #{time_minutes.round} minutes ago"
    elsif time_minutes > 60 && time_minutes < 1440
      time_hours = (time_minutes / 60).round
      "#{user.first_name} created \"#{name}\" — #{time_hours} #{time_hours == 1 ? 'hour' : 'hours'} ago"
    else
      "#{user.first_name} created \"#{name}\" — #{created_at.strftime('%b %d, %Y')}"
    end
  end
end
